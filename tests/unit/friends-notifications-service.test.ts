...
function createFriendsSupabaseMock() {
  return {
    ...
    rpc(name: string, args: Record<string, unknown>) {
      if (name === "create_friend_request_with_notification") {
        expect(args).toEqual({
          p_addressee_id: addresseeId,
          p_actor_label: "@fan_a",
          p_body: "@fan_a chce dodać Cię do znajomych.",
        });
        return Promise.resolve({ data: pendingRequestRow, error: null });
      }

      if (name === "respond_friend_request_with_notification") {
        expect(args).toEqual({
          p_request_id: requestId,
          p_status: "accepted",
          p_actor_label: "@fan_b",
          p_accept_body: "@fan_b zaakceptował(a) Twoje zaproszenie do znajomych.",
        });
        return Promise.resolve({ data: acceptedRequestRow, error: null });
      }
