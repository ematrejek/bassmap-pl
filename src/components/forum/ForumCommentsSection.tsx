...
export default function ForumCommentsSection({
  threadId,
  initialComments,
  isLoggedIn,
  isAdmin,
  ...
  function canDeleteComment(comment: ForumComment): boolean {
    if (isAdmin) {
      return true;
    }
  ...
  function deleteCommentUrl(commentId: string): string {
    return isAdmin ? `/api/admin/forum-comments/${commentId}` : `/api/fan/forum-comments/${commentId}`;
  }
  ...
  async function handleSubmit(event: SubmitEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (typeof data === "object" && data !== null && "comment" in data) {
        const comment = (data as { comment: ForumComment }).comment;
    ...
    <section className="border-border space-y-4 border-t pt-6">
      ...
      {isLoggedIn ? (
        <form className="space-y-3" onSubmit={(event) => void handleSubmit(event)}>
          <div className="space-y-2">
            <Label htmlFor="forum-comment-body" className="text-foreground/90">
              Twój komentarz
            </Label>
            <Textarea
      ...
      {pendingDeleteId !== null ? (
        <div
          ...
          role="alertdialog"
          aria-labelledby="delete-forum-comment-title"
          aria-describedby="delete-forum-comment-description"
        >
          <div className="space-y-2">
            <p id="delete-forum-comment-title" className="text-foreground font-medium">
              Usunąć komentarz?
            </p>
            <p id="delete-forum-comment-description" className={cn("text-sm", shellTextMuted)}>
              Ta operacja jest trwała. Komentarz zniknie z wątku.
            </p>
