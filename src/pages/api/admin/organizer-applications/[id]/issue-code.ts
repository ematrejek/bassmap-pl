...
import { issueVerificationCode } from "@/lib/services/organizer-applications";
...
export const POST: APIRoute = async (context) => {
  ...
  const result = await issueVerificationCode(supabase, idResult.data);
  if ("error" in result) {
    const status = result.error === "Nie znaleziono wniosku" ? 404 : 400;
    return jsonResponse({ error: result.error }, status);
  }

  return jsonResponse({ code: result.data.code }, 200);
};
